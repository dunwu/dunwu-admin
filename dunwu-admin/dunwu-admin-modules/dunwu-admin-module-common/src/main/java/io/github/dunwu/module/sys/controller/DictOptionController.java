...
public class DictOptionController {
    ...
    @AppLog(bizType = "数据字典详情", operType = "添加", value = "'向 sys_dict_option 表中添加一条记录，内容为：' + #entity")
    ...
    @AppLog(bizType = "数据字典详情", operType = "批量添加", value = "'向 sys_dict_option 表中批量添加 ' + #list.size + ' 条记录'")
    @PreAuthorize("@exp.check('sys:dict:add')")
    ...
    @AppLog(bizType = "数据字典详情", operType = "更新", value = "'更新 sys_dict_option 表中 id = ' + #entity.id + ' 的记录，内容为：' + #entity")
    ...
    @AppLog(bizType = "数据字典详情", operType = "批量更新", value = "'批量更新 sys_dict_option 表中 ' + #list.size + ' 条记录'")
    @PreAuthorize("@exp.check('sys:dict:edit')")
    ...
    @AppLog(bizType = "数据字典详情", operType = "删除", value = "'删除 sys_dict_option 表中 id = ' + #entity.id + ' 的记录'")
    ...
    @AppLog(bizType = "数据字典详情", operType = "批量删除", value = "'批量删除 sys_dict_option 表中 ' + #list.size + ' 条记录'")
    @PreAuthorize("@exp.check('sys:dict:del')")
    ...
    @AppLog(bizType = "数据字典详情", operType = "导出", value = "'导出 sys_dict_option 表中 id = ' + #ids + ' 的记录'")
