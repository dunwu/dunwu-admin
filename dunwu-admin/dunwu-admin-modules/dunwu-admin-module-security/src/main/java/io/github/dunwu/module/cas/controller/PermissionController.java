 ...
 */
@RestController
@RequestMapping("/cas/permission")
@Api(tags = "权限表 Controller 类")
...
public class PermissionController {
    ...
    @AppLog(bizType = "权限表", operType = "添加", value = "'向 cas_permission 表中添加一条记录，内容为：' + #entity")
    @PreAuthorize("@exp.check('cas:permission:add')")
    @PostMapping("/add")
    ...
    @AppLog(bizType = "权限表", operType = "批量添加", value = "'向 cas_permission 表中批量添加 ' + #list.size + ' 条记录'")
    @PreAuthorize("@exp.check('cas:permission:add')")
    ...
    @AppLog(bizType = "权限表", operType = "更新", value = "'更新 cas_permission 表中 id = ' + #entity.id + ' 的记录，内容为：' + #entity")
    @PreAuthorize("@exp.check('cas:permission:edit')")
    @PostMapping("/edit")
    ...
    @AppLog(bizType = "权限表", operType = "批量更新", value = "'批量更新 cas_permission 表中 ' + #list.size + ' 条记录'")
    @PreAuthorize("@exp.check('cas:permission:edit')")
    ...
    @AppLog(bizType = "权限表", operType = "删除", value = "'删除 cas_permission 表中 id = ' + #entity.id + ' 的记录'")
    @PreAuthorize("@exp.check('cas:permission:del')")
    @PostMapping("/del/{id}")
    ...
    @AppLog(bizType = "权限表", operType = "批量删除", value = "'批量删除 cas_permission 表中 id = ' + #ids + ' 的记录'")
    @PreAuthorize("@exp.check('cas:permission:del')")
    ...
    @PreAuthorize("@exp.check('cas:permission:view')")
    @GetMapping("/list")
    ...
    @AppLog(bizType = "权限表", operType = "导出", value = "'导出 cas_permission 表中 id = ' + #ids + ' 的记录'")
    @PreAuthorize("@exp.check('cas:permission:view')")
    ...
    @AppLog(bizType = "权限表", operType = "导出", value = "分页导出 cas_permission 表中的记录")
    @PreAuthorize("@exp.check('cas:permission:view')")
