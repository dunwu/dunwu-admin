...
public class AppController {
    ...
    @AppLog(bizType = "应用配置", operType = "添加", value = "'向 mnt_app 表中添加一条记录，内容为：' + #entity")
    ...
    @AppLog(bizType = "应用配置", operType = "批量添加", value = "'向 mnt_app 表中批量添加 ' + #list.size + ' 条记录'")
    @PreAuthorize("@exp.check('mnt:app:add')")
    ...
    @AppLog(bizType = "应用配置", operType = "更新", value = "'更新 mnt_app 表中 id = ' + #entity.id + ' 的记录，内容为：' + #entity")
    ...
    @AppLog(bizType = "应用配置", operType = "批量更新", value = "'批量更新 mnt_app 表中 ' + #list.size + ' 条记录'")
    @PreAuthorize("@exp.check('mnt:app:edit')")
    ...
    @AppLog(bizType = "应用配置", operType = "删除", value = "'删除 mnt_app 表中 id = ' + #entity.id + ' 的记录'")
    ...
    @AppLog(bizType = "应用配置", operType = "批量删除", value = "'批量删除 mnt_app 表中 ' + #list.size + ' 条记录'")
    @PreAuthorize("@exp.check('mnt:app:del')")
    ...
    @AppLog(bizType = "应用配置", operType = "导出", value = "'导出 mnt_app 表中 id = ' + #ids + ' 的记录'")
    @PreAuthorize("@exp.check('mnt:app:view')")
    ...
    @AppLog(bizType = "应用配置", operType = "导出", value = "分页导出 mnt_app 表中的记录")
    @PreAuthorize("@exp.check('mnt:app:view')")
